...
function validateAuthFactorInfo(request: AuthFactorInfo): void {
  ...
  if (typeof request.totpInfo !== undefined) {
    // totpInfo must be a valid Totp Object
    if (!validator.isTotpObject(request.totpInfo)) {
      throw new FirebaseAuthError(
        AuthClientErrorCode.INVALID_TOTP_OBJECT,
        `The second factor "totpObject" for "${authFactorInfoIdentifier}" must be a TOTP Object` +
        'with valid non empty sharedSecretKey string.');
    }
  } else {
